...
<script>
    ...
    async function fetchGameData() {
        ...
        try {
            const listRes = await fetch(`${githubRawBase}/list_games.json`);
            const listData = await listRes.json();
            
            const gameInfo = listData.find(g => g.slug === slug);

            if (!gameInfo || !gameInfo.folder) {
                document.getElementById('main-content').innerHTML = "Game Data Not Found in Index.";
                return;
            }

            const dataPath = `${githubRawBase}/data/${gameInfo.folder}/${slug}.json`;
            const dataRes = await fetch(dataPath);
            
            if (!dataRes.ok) throw new Error("File detail tidak ditemukan");
            
            const fullData = await dataRes.json();
            renderPage(fullData);
            renderRecommendations(fullData['Link Adsterra'], fullData['Link Monetag'], listData);

        } catch (error) {
            console.error(error);
            document.getElementById('main-content').innerHTML = "Error loading game details.";
        }
    }
    ...
    function renderPage(data) {
        ...
        document.getElementById('main-content').innerHTML = `
            <div class="featured-img-container">
                <img src="${data['Header image']}" class="featured-img" alt="${data['Alt Text Image']}" width="250" height="140">
            </div>
            <table class="specs-table">
                ...
                <tr><td>About Game</td><td>${data['About the game']}</td></tr>
            </table>
        `;
    }

    function renderRecommendations(adsterra, monetag, allGames) {
        ...
        grid.innerHTML = selected.map(g => `
            <div class="card">
                <img src="${g.img}" alt="${g.name}" width="100" height="100" onclick="window.open('${adsterra}', '_blank'); window.location.href='?id=${g.slug}'">
                <div class="card-body">
                    <h4>${g.name}</h4>
                    <button class="btn-small" onclick="window.open('${monetag}', '_blank'); window.location.href='?id=${g.slug}'">View Detail</button>
                </div>
            </div>
        `).join('');
    }
